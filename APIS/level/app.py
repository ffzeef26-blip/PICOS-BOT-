...
def calculate_level_progress(current_exp, current_level):
    ...
    try:
        ...
        current_exp = int(current_exp or 0)
        ...
        exp_in_current_level = max(0, current_exp - exp_for_current)
        exp_range_for_level = max(1, exp_for_next - exp_for_current)

        progress_percentage = min(100, (exp_in_current_level / exp_range_for_level) * 100)
...
def fetch_player_info(uid):
    ...
    try:
        api_url = f"http://localhost:5003/get?uid={uid}"
...
def home():
    return jsonify({
        ...
        "endpoints": {
            "/level?uid=[uid]": "Get level progress for player",
            
            "/levels": "Get all level EXP requirements"
        },
        "credit": "t.me/teamx_legacy"
    })

@app.route('/level', methods=['GET'])
def get_level_info():
    """Get basic level information for a player via query parameter uid"""
    ...
    try:
        ...
        current_level = int(account_info.get("AccountLevel", 0) or 0)
        current_exp = int(account_info.get("AccountEXP", 0) or 0)
...
def get_exp_for_level_api():
    """Get EXP required for a specific level via query parameter"""
    level_number = request.args.get("level")
    if not level_number:
        return jsonify({
            "success": False,
