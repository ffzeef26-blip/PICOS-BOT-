...
region = 'ME'
WHITELISTED_UIDS = {
    "XANAF_LEGACY", # don't change this text
    "563640835"
}
...
uid=7339296682,password=321BE9FE6C33274DF1CB423BBD9EE72B56C738606194F9A5E8E51969D613907A
...
def load_credentials_from_file(filename="TEAMXLG.txt"):
    """
    Load UID and password from TEAMXLG.txt file
    """
    try:
        if not os.path.exists(filename):
...
🤖 Bot Name: TEAM X LEGACY 
✅ Status: Ready to join

📱 Player will see:
"TEAM X LEGACY wants to join your team"
...
async def request_join_with_badge(target_uid, badge_value, key, iv, region="me"):
        ...
        fields = {
            ...
            2: {
                ...
                27: {                    # Field 27 (critical for badge!)
                    1: 11,               # Field 27.1
                    2: 15127479756,      # Field 27.2 (your bot UID)
                    3: 9999              # Field 27.3
                },
                28: {},                   # Empty field 28
...
async def black666(client_id, key, iv):
    banner_text = "[FF0000][B][C] ERROR , WELCOME TO [FFFFFF] PICOS BOT [00FF00]___ TEAMX LG____ BOT ! \n[FFFF00]NEW VERSION NEW FUNCTION !\n[FF0000]TELEGRAM : @picos0\n\n"     
    fields = {
        1: 5,
        2: {
            1: int(client_id),
...
def add_friend(target_uid):
    try:
        # Read credentials from TEAMXLG.txt
        try:
            with open("TEAMXLG.txt", 'r') as f:
...
def send_likes(uid):
    try:
        likes_api_response = requests.get(
             f"https://ob-53-like-api-by-x.vercel.app/like?uid={uid}&server_name=me",
             timeout=15
             )
...
def send_visits(uid):
    try:
        visits_api_response = requests.get(
            f"https://team-x-visit-api.vercel.app/visit?uid={uid}&region=me",
            timeout=30
        )
...
def spam_requests(player_id):
    url = f"https://teamx-spam.vercel.app/spam?uid={player_id}&region=me"
...
📝 Example: /spamroom 123456 15127479756 Hello World!
...
async def auto_reply_with_emote(emote_info, key, iv):
    ...
    try:
        ...
        bot_uid = 15127479756  # Replace with your bot's actual UID
              ...
              # =================== EMOTE DETECTION ONLY ===================
                ...
                if data_hex.startswith("0500") and insquad is None and joining_team == False:
                    try:
                        ...
                        if "XANAF_LEGACY" in WHITELISTED_UIDS:
                ...
                if insquad == True:
                    try:
                        ...
                        message = """[َb][َc][َi]✿[00FFFFَ]᭄ᭃ ♡السلام عليكم[َb][َc][َi]✿[00FFFFَ]᭄ᭃ ♡"""
...
# ========== লেন্থ 0 হলে ডুয়েল ইমোট পাঠান ==========
                        if len(response.Data.msg) == 0:
                            ...
                            print(f"🤖 Selected emote: {selected_emote}")
                            
                            bot_uid = 15127479756  # আপনার বটের UID
                            ...
                            try:
                                ...
                                if user_packet and online_writer:
                                    ...
                                    print(f"✅ Emote to the user ({selected_emote}) Sent")
                                ...
                                if bot_packet and online_writer:
                                    ...
                                    print(f"✅ The bot sent the same emote to itself.")
                        ...
                        # NEW LAG COMMAND
                        if inPuTMsG.strip().startswith('/laghho'):
                        ...
                        # STOP LAG COMMAND
                        if inPuTMsG.strip() == '/stop laghovk':
                            if lag_task and not lag_task.done():
                                lag_running = False
                                lag_task.cancel()
                        ...
                        if inPuTMsG.strip().startswith('/rejectyuhi'):
                        ...
                        if inPuTMsG.strip() == '/reject_stophhhh':
                            if reject_spam_task and not reject_spam_task.done():
                                reject_spam_running = False
                                reject_spam_task.cancel()
...
"""
                                ...
                                credentials = load_credentials_from_file("TEAMXLG.txt")
                                if not credentials:
...
[1E90FF]━━━━━━━━━━━━━━
[32CD32]{subject} Who am I giving it to? {rating}Rating /100
[1E90FF]━━━━━━━━━━━━━━
[FFFF00]💡 Fun rating, anything is possible, whether real or imaginary!
[1E90FF]━━━━━━━━━━━━━━"""
                        ...
                        if inPuTMsG.strip().startswith('/wlremove'):
                            ...
                            # Check owner
                            if str(response.Data.uid) != "563640835":
...
"""
                            ...
                            if str(response.Data.uid) != "563640835":  # Replace with your actual UID
...
"""
                        ...
                        if inPuTMsG.strip() == '/wllist':
                            ...
                            # Check if owner
                            if str(response.Data.uid) != "563640835":  # Your UID
...
👥 Whitelisted UIDs:"""
                            ...
                            for uid in WHITELISTED_UIDS:
                                if uid != "15127479756":  # Skip owner since already shown
                                    whitelist_msg += f"\n• {xMsGFixinG(uid)}"
                                    count += 1
                                    if count >= 20:
...
📝 Examples:
/kkick {xMsGFixinG('15127479756')} - Kick single player with "Connection Lost"
/kkick {xMsGFixinG('15127479756')} {xMsGFixinG('123456789')} - Kick multiple players
...
[B][FFD700]💬 Bot Available for sale
[B][32CD32]⚡ Fast, Powerful & Secure
[B][00FFFF]🌟 Usable in all types of gaming/team chats
[B][FF1493]🔥 To buy, contact me on Telegram
[B][FFD700]📌 @Picos_dv
...
"""
                        ...
                        if inPuTMsG.strip().startswith("/dare"):
                            dares = [
    "🗣️ Sing loudly with a friend for 30 seconds!",
    "😜 Hold your phone in front of your face and do a funny makeup!",
    "😂 Make a funny meme and show it to everyone!",
    "🤪 Cry like a baby for 20 seconds!",
    "😎 Give a strange compliment to a random friend!",
    "🫣 Dramatically propose to someone you like by saying their name!",
    "🥳 Do a weird dance for 1 minute, as funny as possible!",
    "🤭 Tell the most embarrassing jokes and make everyone laugh!"
]
                            selected_dare = random.choice(dares)

                            msg = f"""
[B][C][FF69B4]━━━━━━━━━━━━━━━━━━━━━
[FF4500]🎲 Vhonkor's dare 🎲
[FF69B4]━━━━━━━━━━━━━━━━━━━━━
[7B68EE]{selected_dare}
[FF69B4]━━━━━━━━━━━━━━━━━━━━━
[FFFF00]⚡ Show your courage, you have to do the dare! ⚡
[FF69B4]━━━━━━━━━━━━━━━━━━━━━"""
                            await safe_send_message(response.Data.chat_type, msg, uid, chat_id, key, iv)
...
"""
                        ...
                        # Update the command handler
                        if inPuTMsG.strip().startswith('/rejectjkfo'):
                            print('Processing reject spam command in any chat type')
                        ...
                        if inPuTMsG.strip() == '/reject_stophhhh':
                            if reject_spam_task and not reject_spam_task.done():
                                ...
                                #GET PLAYER INFO
                        if inPuTMsG.strip().startswith('/infogujh'):
                            print('Processing bio command in any chat type')
...
"""
                                ...
                                try:
                                    url = f"https://rizerxghostbd.onrender.com/x/{teamcode}/PICOS_BOT"
...
# NEW LAG COMMAND (AUTO STOP AFTER 10 SECONDS)
                        if inPuTMsG.startswith('/PICOSBOTerror'):
                                ...
                                if len(parts) < 2:
                                    ...
                                    for blocked_name in BLOCKED_NAMES:
                                        ...
                                        if normalized_blocked in normalized_name:
                                            ...
                                            break
                                    if blocked:
                                        error_msg = '[B][C][FF0000]⚠️ Damn, you mot😂her🤐f🤣uc😂ker🥴\n[FFFFFF]You want to insult {Name}’s fa🤣ther\n[FF0000]Da😂mn, you mot😂her🤣f😁uc😂ker'
                                        await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                    else:
                                        messages = [
    '{Name} I will forcefully have s😂ex with your se😂xy sis😂ter all night',
    '{Name} I will have se😂x with your mot😂her while she is on top of you',
    '{Name} so😂n of a wh😂ore!!',
    '{Name} mot😂her😂fu😂c😂ker, I will fu😂ck your mot😂her!!',
    '{Name} mot😂herfu😂cker, I will fu😂ck your mot😂her at 5G speed!!',
    '{Name} dumb fu😂ck, I will fu😂ck your mot😂her with a con😂dom on!!',
    '{Name} du😂mb fu😂ck, I provide your mot😂her with a 10,000 taka service every day!!',
    'F🤫U🤫C🤫K {Name}!!',
    '{Name} mot😂he😂rf😂uc😂ker, I will kick your ass!!',
    '{Name} mot😂he😂rf😂uc😂ker!!',
    '{Name} whore, I am your fat😂her!!',
    '{Name} I fu😂c😂ked your mot😂her and gave bir😂th to you!!',
    '{Name} du😂mb fu😂ck, so😂n of a whore!!',
    '{Name} mot😂her😂fu😂ck😂er, I will fu😂ck your mot😂her at 180 km/h!!',
    '{Name} so😂n of a whore bot, noob player!!',
    'MENA NO.1 BOT PLAYER {Name}',
    '{Name} sh😂oe thief!!',
    '{Name} mot😂her😂fu😂ck😂er, stop playing Free Fire and go play Ludo!!',
    '{Name} what😂ever you do, I am your fat😂her, never forget that!!'
]
                                        for msg in messages:
...
# PRAISA COMMAND (17 POSITIVE MESSAGES)
    ...
    "💎 {Name} You are unique, there is no one else like you!",
    "🚀 {Name} Your future is very bright!",
    "👑 {Name} You are worthy of becoming a leader!",
    "🌈 {Name} Your smile makes everyone's day better!",
    "💖 {Name} Always stay this positive!",
    "🏆 {Name} You have the ability to achieve whatever you want!",
    ...
    "🧠 {Name} Your way of thinking is truly admirable!",
    "💫 {Name} You will go very far, Insha🤫Allah!",
                        ...
                        if inPuTMsG.strip().startswith('/love '):
                            ...
                            try:
                                ...
                                if len(parts) < 2:
                                    ...
                                    messages = [
    "{Name} I only love you ♡",
    "{Name} You are my everything ♡",
    "{Name} My heart always beats for you ♡",
    "{Name} You are the light of my life ♡️",
    "{Name} You are the greatest happiness in my life ♡",
    "{Name} You are the closest person to my heart ♡",
    "{Name} You are the person of my dreams ♡",
    "{Name} People like you are very rare",
    "{Name} I can't get through my day without you ♡",
    "{Name} You are the person of my dreams ♡",
    "{Name} I always want to be by your side ♡",
    "{Name} I love you ♡",
    "{Name} My life is incomplete without you ♡",
    "{Name} Your smile lights up my world ♡♡",
    "{Name} I LOVE YOU ♡",
...
# ==================== /luck COMMAND ====================
                        if inPuTMsG.strip() == "/luck":
                            luck_messages = [
    "Today you will get an AWM while looting an airdrop! ⚡",
    "Today you will land critical hits in your ranked match! 🔥",
    "Today you could become a champion! 🏆",
    "Today you will have fun moments with your friends! 😎",
    "Today some things will go well by luck! 🍀"
]
                            selected_message = random.choice(luck_messages)
                            admin_message = (
                                f"[C][B][00FFFF]╔════════════════════╗\n"
                                f"║  ╭──────────────╮\n"
                                f"║  │[FFFF00] Luck Testing Center         │\n"
                                f"║  ╰──────────────╯\n"
                                f"║\n"
                                f"║ [FF69B4]✨ {selected_message}\n"
                                f"║ ────────────────\n"
                                f"║ ⚡ [FF00FF] PICOS VIP BOT ⚡\n"
                                f"╚════════════════════╝"
                            )
...
# ==================== IMPORTS ====================
                        ...
                        if inPuTMsG.strip().startswith("/advice"):
                            ...
                            advice_list = [
    "[B][FFD700]💡 Play carefully today, your rank will increase!",
    "[B][32CD32]🍀 Playing together with the team gives you a 100% chance of success!",
    "[B][FF4500]⚠️ Be careful when going alone, you might get backstabbed!",
    "[B][00FFFF]🌟 Practice headshots, today will be a good day!",
    "[B][FF1493]💖 Don't forget to communicate with your friends!",
    "[B][FF8C00]🏹 You may have a chance to get rare loot today!",
    "[B][9400D3]🛡️ Stay in the safe zone, don't take too many risks!",
    "[B][00FF00]🎯 Be patient, you may get a good kill rate today!",
    "[B][FFA500]🔥 You have a high chance of winning your first game today!",
    "[B][1E90FF]💨 Move quickly, the enemy won't be able to catch you!",
    "[B][FF69B4]🌸 Team up with your friends and collect loot!",
    "[B][ADFF2F]🌟 You have a chance to get rare skin loot today!",
    "[B][FF0000]💀 Be careful! You might get backstabbed!",
    "[B][00CED1]🛡️ Stay in a safe place, the squad's risk will be lower!",
    "[B][FF4500]⚡ Today is the day to increase your headshot rate!"
]
...
# In TcPChaT function, update /status command:
                        ...
                        if inPuTMsG.strip().lower() in ("help","/help","hi","hello","commands"):
                            ...
                            p1 = """[C][B][00FFFF]════════════
PICOS COMMUNITY HELP MENU 1
════════════

[FF0000]1 [FFFFFF]Show Help Manu
❀️ [00FFFF]/help 

[FFFF00]2 [FFFFFF]Get YouTube Info
❀️ [00FFFF]/yt [name]

[FFA500]3 [FFFFFF]Room Id If Player In Room
❀️ [00FFFF]/roomid [uid]

[00FFFF]4 [FFFFFF]Send Likes
❀️ [00FFFF]/like [uid]
"""
                            ...
                            p2 = """[C][B][FF00FF]════════════
PICOS COMMUNITY HELP MENU 2
════════════

[FF4500]5 [FFFFFF]Leave From Group
❀️ [FF00FF]/exit

[FF0000]6 [FFFFFF]Join Team
❀️ [FF00FF]Just Send Team Code

[00FF00]7 [FFFFFF]Account Ban Check
❀️ [FF00FF]/check [uid]

[FFFF00]8 [FFFFFF]Make 5 Player Group
❀️ [FF00FF]/5

[FFA500]9 [FFFFFF]Make 3 Player Group
❀️ [FF00FF]/3
"""
                            ...
                            p3 = """[C][B][00FF00]════════════
PICOS COMMUNITY HELP MENU 3
════════════

[FF0000]10 [FFFFFF]Make 6 Player Group
❀️ [00FF00]/6

[00FF00]11 [FFFFFF]Ban Player
❀️ [00FF00]/ban [uid]

[FFFF00]12 [FFFFFF]Unban Player 
❀️ [00FF00]/unban [uid]

[FFA500]13 [FFFFFF]Try Your Luck
❀️ [00FF00]/luck

[00FFFF]14 [FFFFFF]Gali Any Friend 
❀️ [00FF00]/gali [name]
"""
                            ...
                            p4 = """[C][B][00BFFF]════════════
PICOS COMMUNITY HELP MENU 4
════════════

[FF00FF]15 [FFFFFF]Praise Any Player
❀️ [00BFFF]/praise [name]

[FF4500]16 [FFFFFF]Love Msg To Person 
❀️ [00BFFF]/love [name]

[00FF00]17 [FFFFFF]Add Bot In Friend 
❀️ [00BFFF]/add [uid]

[FFFF00]18 [FFFFFF]Remove From Friend 
❀️ [00BFFF]/remove [uid]
"""
                            ...
                            p5 = """[C][B][32CD32]════════════
PICOS COMMUNITY HELP MENU 5
════════════

[FF0000]19 [FFFFFF]Dice Roll of Ludo
❀️ [32CD32]/dice

[00FF00]20 [FFFFFF]Coin Flip Head or Tell
❀️ [32CD32]/coin

[FFFF00]21 [FFFFFF]Evo Emote Cycle 
❀️ [32CD32]@evos

[FFA500]22 [FFFFFF]Stop Evo Emote Cycle 
❀️ [32CD32]@sevos

[00FFFF]23 [FFFFFF]Play Emote
❀️ [32CD32]/e [emote]
"""
                            ...
                            p6 = """[C][B][FF1493]════════════
PICOS COMMUNITY HELP MENU 6
════════════

[FF00FF]24 [FFFFFF]Quick 25x Emote
❀️ [FF1493]/fast [uids] [emote]

[FF4500]25 [FFFFFF]Emote Spam
❀️ [FF1493]/es [uids] [emote]

[FF0000]26 [FFFFFF]Emote Without Bot 
❀️ [FF1493]/em [teamcode] [uids] [emote

[FFFF00]27 [FFFFFF]Start Match
...
"""
                            ...
                            p7 = """[C][B][FF69B4]════════════
PICOS COMMUNITY HELP MENU 7
════════════

[00FF00]28 [FFFFFF]Play Emote Using Id
❀️ [FF69B4]/play [uid] [emote_id]

[FFFF00]29 [FFFFFF]Freeze Player Using Emote
❀️ [FF69B4]/freeze [uid]

[FFA500]30 [FFFFFF]Set Welcome Emote
...
"""
                            ...
                            p8 = """[C][B][00FA9A]════════════
PICOS COMMUNITY HELP MENU 8
════════════

[FF0000]31 [FFFFFF]Get Bundle List
❀️ [00FA9A]/bundle

[00FF00]32 [FFFFFF]Equip Bundle To Bot
❀️ [00FA9A]/bundle [name]

[FFFF00]33 [FFFFFF]Reply of Noob
❀️ [00FA9A]noob

[FFA500]34 [FFFFFF]Show Rare Titles
❀️ [00FA9A]/title
"""
                            ...
                            p9 = """[C][B][8A2BE2]════════════
PICOS COMMUNITY HELP MENU 9
════════════

[FFA500]35 [FFFFFF]Get Player Status
❀️ [8A2BE2]/status [uid]

[00FFFF]36 [FFFFFF]Couut Emote Play
...
"""
                            ...
                            p10 = """[C][B][00FF7F]════════════
PICOS COMMUNITY HELP MENU 10
════════════

[FF0000]37 [FFFFFF]Craftland Badge Spam
❀️ [00FF7F]/s1 [uid]

[00FF00]38 [FFFFFF]V Badge Spam
❀️ [00FF7F]/s2 [uid]

[FFFF00]39 [FFFFFF]Modarator Badge Spam
❀️ [00FF7F]/s3 [uid]

[FFA500]40 [FFFFFF]Old V Badge Spam
❀️ [00FF7F]/s4 [uid]

[00FFFF]41 [FFFFFF]Pro Badge Spam
❀️ [00FF7F]/s5 [uid]
"""
                            ...
                            p11 = """[C][B][32CD32]════════════
PICOS COMMUNITY HELP MENU 11
════════════

[00FF00]42 [FFFFFF]See Ship In Person 
❀️ [FF1493]/ship [person1] [person2]

[FFFF00]43 [FFFFFF]Random D A R E
❀️ [FF1493]/dare

[FFA500]44 [FFFFFF]Rate Anything 
❀️ [FF1493]/rate [subject]

[00FFFF]45 [FFFFFF]See Advice Game Releted
❀️ [FF1493]/advice
"""
                            ...
                            p12 = """[C][B][8A2BE2]════════════
PICOS COMMUNITY HELP MENU 12
════════════

[00FF00]46 [FFFFFF]See Admin Info
❀️ [8A2BE2]/admin

[FFA500]47 [FFFFFF]Emote Like Spam
❀️ [8A2BE2]/es [uid]

[00FFFF]48 [FFFFFF]Count Emote Play
❀️ [8A2BE2]/p [uid] [emote] [number]
"""
                            ...
                            p13 = """[C][B][00FF7F]════════════
PICOS COMMUNITY HELP MENU 13
════════════

[FF0000]49 [FFFFFF]Get Animation List
❀️ [00FF7F]/animation 

[00FF00]50 [FFFFFF]Play Animation 
❀️ [00FF7F]/animation [anim]

[FFFF00]51 [FFFFFF]Look Changer List
❀️ [00FF7F]/look

[FFA500]52 [FFFFFF]Play Look Changer 
❀️ [00FF7F]/look [lookchanger]

[00FFFF]53 [FFFFFF]Auto Look Changes 
...
Bᴀsɪᴄ ᴀɴᴅ Aɴʏ_Rᴏʟʟs
════════════[FFFFFF][B]
  ❀️ The guild is not for personal benefit — it is a family, and it is everyone's responsibility to grow the family together.

[FFFF00]❀️ Everyone is equal in the guild – brothers, friends, no fighting. We will stay united and support each other.

[00FFFF]We will work together, and we will win together!!

[B][C][FF69B4] Guild Rule: Every member must contribute ❀️ 10,000 Glory every week.
[FFFF00]════════════
"""
...
⚡Aᴅᴍɪɴ Cᴏᴍᴍᴀɴᴅꜱ⚡
════════════[7CFC00][B]i
If you want to buy the bot contact me❀️

[FFFFFF]INSTAGRAM & [00FFFF]TELEGRAM 

[FF00FF] ❀️ TELIGRAM GROUP:
[FFFF00] ㅤ////////

[FFFFFF]❀️ teligram ᴀɴᴅ Sᴜᴘᴘᴏʀᴛ: 
[00FFFF] ㅤ@Picos_dv
...
async def MaiiiinE():
    ...
    print("📁 Loading credentials from TEAMXLG.txt")
    credentials = load_credentials_from_file("TEAMXLG.txt")
    
    if not credentials:
        print("❌ Failed to load credentials!")
        print("💡 Please create TEAMXLG.txt with your UID and password")
